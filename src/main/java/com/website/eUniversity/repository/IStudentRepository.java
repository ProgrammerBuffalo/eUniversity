...
public interface IStudentRepository extends JpaRepository<Student, Integer> {
    ...
    @Query(value = "SELECT new com.website.eUniversity.model.dto.admin_panel.entity.StudentDTO(a.id, s.id, a.fullName, a.age, a.login, a.password, g.name)" +
    ...
    @Query(value = "SELECT new com.website.eUniversity.model.dto.admin_panel.entity.StudentDTO(s.account.id, s.id, s.account.fullName, s.account.age, s.account.login, s.account.password)" +
    ...
    @Query(value = "SELECT COUNT(*) FROM students s INNER JOIN accounts acc on acc.id = s.account_id where acc.full_name LIKE %:search% OR acc.login LIKE %:search%",
            nativeQuery = true)
